...
import day19.Day19Puzzle1.Scanner;
import day19.Day19Puzzle2.Scanner.MatchResult;
...
public class Day19Puzzle2 {
	...
	public static class Scanner {
		...
		Set<Vector> scanners = new HashSet<>();
...
//					for (Vector t : translatedBeacons) {
//						for (Vector b : beacons) {
//							if (t.distance(b) == 0) {
//								numberOfMatches++;
//							}
...
//						+ matchResult.numberOfMatches);
		...
		public void merge(Scanner otherScanner, MatchResult matchResult) {
			...
			scanners.add(matchResult.t);
		}
	}

	static long manhattanDistance(Vector v1, Vector v2) {
		return Math.abs(v1.x - v2.x) + Math.abs(v1.y - v2.y) + Math.abs(v1.z - v2.z);
	}

	public static void main(String[] args) throws IOException, URISyntaxException {
		List<String> inputLines = Files.readAllLines(Paths.get(Day19Puzzle1.class.getResource("input.txt").toURI()));
		...
		long maxDistance = 0;
		Vector maxV1 = null, maxV2 = null;
		for (Vector b1 : scanner.scanners) {
			for (Vector b2 : scanner.scanners) {
				long distance = manhattanDistance(b1, b2);
		...
		System.out.println("distance: " + maxDistance + "  " + maxV1 + "  " + maxV2);
	}
}
